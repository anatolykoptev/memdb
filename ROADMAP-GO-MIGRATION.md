...
| **RabbitMQ удалён** | docker-compose.yml: сервис rabbitmq, том rabbitmq_data, dependency и env vars удалены | `deploy-config/docker-compose.yml` |
...
- `deploy-config/docker-compose.yml` — rabbitmq сервис, том и dependency удалены
