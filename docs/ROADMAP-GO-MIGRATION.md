...
> - [docs/backlog/search.md](docs/backlog/search.md) — качество поиска, D1-D11 retrieval features, LoCoMo benchmarks
> - [docs/backlog/add-pipeline.md](docs/backlog/add-pipeline.md) — качество add pipeline, soft-delete, OTel
> - [docs/backlog/features.md](docs/backlog/features.md) — новые фичи (Image Memory, MemCube sharing)
...
### Сервисы (docker-compose) — pure-Go стек после M9 Phase 5

| Сервис | Роль | Порт | Статус |
|--------|------|------|--------|
| **memdb-go** | Go API Gateway: auth, embedder client, search, MCP, scheduler | 8080 | ✅ Go |
| **memdb-mcp** | Go MCP server (stdio + HTTP) | 8001 | ✅ Go |
| **embed-server** | Rust sidecar: multilingual-e5-large + jina-code-v2 + bge-rerank | 8082 | ✅ Rust |
| **postgres+AGE** | Graph DB (pgvector 0.8 + Apache AGE 1.7 + halfvec HNSW) | 5432 | ✅ |
| **qdrant** | Sparse + dense vector store | 6333 | ✅ |
| **redis** | Cache + Streams queue + VSET hot WM cache | 6379 | ✅ |
| ~~memdb-api~~ | Python legacy gateway | ~~8000~~ | ❌ **REMOVED 2026-04-26 (M9 Phase 5)** |

**6 контейнеров** (было 7). Python runtime больше не в стеке.
...
| `mem_agent/deepsearch_agent.py` | 14.4K | QueryRewriter + Reflection deep-search agent | ❌ нет | ❌ (docs/backlog/search.md) |
| `memories/textual/tree_text_memory/retrieve/` | 13 файлов, ~140K | advanced_searcher, bm25_util, bochasearch, internet_retriever, pre_update, reasoner, recall, searcher, searxng_search, task_goal_parser, xinyusearch, retrieve_utils | ⚠️ частично (searxng + searcher + internet_retriever) | ❌ (docs/backlog/search.md) |
...
| `reranker/strategies/http_bge.py` + `http_bge_strategy.py` | 23K | BGE HTTP reranker (4 стратегии) | ❌ нет (только LLM-rerank) | ❌ (docs/backlog/search.md) |
...
| Post-retrieval memory enhancement | ✅ | ❌ → docs/backlog/search.md | 🔴 |
| 3-stage iterative retrieval prompts | ✅ (3 staged prompts) | ❌ (1 flat prompt) | 🔴 |
| CoT query decomposition | ✅ | ❌ → docs/backlog/search.md | 🔴 |
| Query rewriting before embedding | ✅ | ❌ → docs/backlog/search.md | 🟡 |
...
## Метрики успеха — post-M9 actuals

| Фаза | KPI | Цель | Факт (2026-04-26) |
|------|-----|------|-------------------|
| 4 | Extraction tests coverage | ≥ 80% | ~70% (livepg + unit, gap в edge cases) |
| **5** | **Python containers** | **0** | ✅ **0** (memdb-api removed) |
| 5 | Add latency p95 | < 1s | ✅ M7 дал -52% p95 чрез factual prompt; window=512: 13s → 1.0s через embed batching (M7) |
| — | Files > 200 lines | 0 | 25 файлов (было 40 до Phase C splits — продолжаем работу) |
| — | Health score (handlers) | ≥ 85 | 79 (полу-A, не растёт без splits) |

---

## Post-shutdown architecture (2026-04-26)

```
┌──────────────────────────────────────────────────┐
│  Client (Claude / vaelor / oxpulse-chat / SDK)   │
└────────────────┬─────────────────────────────────┘
                 │ HTTP / MCP
                 ↓
        ┌────────────────────┐
        │   memdb-go :8080   │  ← single Go binary
        │   memdb-mcp :8001  │
        └──────┬─────────────┘
               │
   ┌───────────┼─────────────────────────┐
   ↓           ↓             ↓           ↓
┌────────┐ ┌───────┐ ┌──────────────┐ ┌──────────────┐
│postgres│ │redis  │ │qdrant        │ │embed-server  │
│+pgvec  │ │+VSET  │ │sparse+dense  │ │(Rust)        │
│+AGE    │ │+streams│ │              │ │e5+jina+rerank│
└────────┘ └───────┘ └──────────────┘ └──────────────┘
```

Точно **6 контейнеров**. Никакого Python в hot path. Никаких proxy fallback'ов: при недоступности Postgres → HTTP 503 (deterministic), не 502 dead-upstream через Python (как раньше при сбое мог быть).

**LLM proxy** (cliproxyapi:8317) — отдельный shared сервис, не часть MemDB stack. memdb-go обращается к нему через `LLM_API_BASE` для extract/chat/judge.

---

## Что **дальше делать** (НЕ migration, активные roadmap'ы)

Migration ✅ закрыт. Дальше — качество и фичи:

| Roadmap | Что внутри |
|---------|-----------|
| **[docs/backlog/search.md](docs/backlog/search.md)** | D1-D11 retrieval features (multi-hop, CoT decomp, staged retrieval, post-retrieval enhancement, structural edges), LoCoMo benchmarks, Memobase port targets |
| **[docs/backlog/add-pipeline.md](docs/backlog/add-pipeline.md)** | Pronoun/temporal/third-person resolution в extraction, structured preference taxonomy, hierarchical reorganizer (D3) |
| **[docs/backlog/features.md](docs/backlog/features.md)** | Image Memory, MemCube sharing, новые API surface |

### M10 candidates (из M9 backlog)
- **Structured user_profiles layer** (Memobase moat, ~3-4 weeks): topic/sub_topic/memo first-class table, queryable отдельно от raw memories. Это закрывает Memobase architectural advantage на cat-1 (single-hop) и cat-4 (open-domain).
- **Pre-compute CE rerank scores at ingest** (perf, M effort): persist в `Memory.properties->>'ce_score_topk'`, query-time → graph lookup. Ожидаем -50-300ms p95 chat.
- **PageRank на memory_edges** (perf, S effort): boost в D1 rerank, +recall на cat-1 + cat-3.
- **BulkCopyInsert / CypherWriter для AGE writes** (perf, M effort): 2-5× speedup на heavy ingest paths.

См. `docs/competitive/2026-04-26-memobase-deep-dive.md` для полного M10 design space.
