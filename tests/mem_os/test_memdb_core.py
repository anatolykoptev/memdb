...
@pytest.fixture
def mock_config():
    """Create a mock MemDB config for testing."""
    return {
        "user_id": "test_user",
        "chat_model": {
            "backend": "huggingface",
...
class TestMemDBInitialization:
    """Test MemDB initialization and basic setup."""
    ...
    def test_mos_init_success(
        ...
        """Test successful MemDB initialization."""
        # Setup mocks
        ...
        config = MemDBConfig(**mock_config)
        mos = MemDBCore(config)
    ...
    def test_mos_init_invalid_user(self, mock_llm_factory, mock_user_manager_class, mock_config):
        """Test MemDB initialization with invalid user."""
...
class TestMemDBUserManagement:
    """Test MemDB user management functions."""
    ...
    def test_create_user(
        ...
        mos = MemDBCore(MemDBConfig(**mock_config))
...
class TestMemDBMemoryOperations:
    """Test MemDB memory operations."""
    ...
    @patch("memdb.mem_os.core.MemReaderFactory")
    @patch("memdb.mem_os.core.LLMFactory")
    @patch("memdb.mem_os.core.logger")
    def test_register_mem_cube_embedder_consistency_warning(
        ...
        """Test embedder consistency warning when cube embedder differs from MemDB config."""
        # Setup mocks
        mock_llm_factory.from_config.return_value = mock_llm
        mock_reader_factory.from_config.return_value = mock_mem_reader
        mock_user_manager_class.return_value = mock_user_manager
        ...
        # Create different embedder configs for MemDB and cube
        mos_embedder_config = {
            "backend": "ollama",
            "config": {
                "model_name_or_path": "nomic-embed-text:latest",
        ...
        with patch.object(GeneralMemCube, "init_from_dir", return_value=mock_mem_cube):
            ...
            # Ensure MemDB config has different embedder
            ...
            mock_logger.warning.assert_called_with(
                f"Cube Embedder is not consistent with MemDBConfig for cube: test_cube_1, will use Cube Embedder: {cube_embedder_config}"
            )
    ...
    def test_register_mem_cube_embedder_consistency_no_warning(
        ...
        """Test no warning when cube embedder is consistent with MemDB config."""
        # Setup mocks
        mock_llm_factory.from_config.return_value = mock_llm
        mock_reader_factory.from_config.return_value = mock_mem_reader
        mock_user_manager_class.return_value = mock_user_manager
        ...
        # Create same embedder config for both MemDB and cube
        embedder_config = {
            "backend": "ollama",
            "config": {
                "model_name_or_path": "nomic-embed-text:latest",
        ...
        with patch.object(GeneralMemCube, "init_from_dir", return_value=mock_mem_cube):
            ...
            # Ensure MemDB config has same embedder
...
class TestMemDBChat:
    ...
    def test_chat_without_memories(
        ...
        mos = MemDBCore(MemDBConfig(**config_dict))
...
class TestMemDBSystemPrompt:
    """Test the _build_system_prompt method in MemDBCore."""

    @pytest.fixture
    def mos_core_instance(self, mock_config, mock_user_manager):
        """Fixture to create a MemDBCore instance for testing the prompt builder."""
        with patch("memdb.mem_os.core.LLMFactory"), patch("memdb.mem_os.core.MemReaderFactory"):
            return MemDBCore(MemDBConfig(**mock_config), user_manager=mock_user_manager)
...
class TestMemDBErrorHandling:
    """Test MemDB error handling."""
