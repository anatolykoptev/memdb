...
from memdb.mem_os.product import MemDBProduct
from memdb.memdb_tools.notification_service import get_error_bot_function, get_online_bot_function
...
# Initialize MemDBProduct instance with lazy initialization
MEMDB_PRODUCT_INSTANCE = None


def get_mos_product_instance():
    """Get or create MemDBProduct instance."""
    global MEMDB_PRODUCT_INSTANCE
    if MEMDB_PRODUCT_INSTANCE is None:
        default_config = APIConfig.get_product_default_config()
        logger.info(f"*********init_default_mos_config********* {default_config}")
        from memdb.configs.mem_os import MemDBConfig

        mos_config = MemDBConfig(**default_config)
        ...
        MEMDB_PRODUCT_INSTANCE = MemDBProduct(
            default_config=mos_config,
            default_cube_config=default_cube_config,
            online_bot=online_bot,
            error_bot=error_bot,
        )
        logger.info("MemDBProduct instance created successfully with inheritance architecture")
    return MEMDB_PRODUCT_INSTANCE


get_mos_product_instance()


@router.post("/configure", summary="Configure MemDBProduct", response_model=SimpleResponse)
def set_config(config):
    """Set MemDBProduct configuration."""
    global MEMDB_PRODUCT_INSTANCE
    MEMDB_PRODUCT_INSTANCE = MemDBProduct(default_config=config)
...
def create_memory(memory_req: MemoryCreateRequest):
    ...
    logger.debug("DIAGNOSTIC: /product/add endpoint called.")
...
@router.post("/chat", summary="Chat with MemDB")
def chat(chat_req: ChatRequest):
    """Chat with MemDB for a specific user. Returns SSE stream."""
    try:
        mos_product = get_mos_product_instance()
...
@router.post("/chat/complete", summary="Chat with MemDB (Complete Response)")
def chat_complete(chat_req: ChatCompleteRequest):
    """Chat with MemDB for a specific user. Returns complete response (non-streaming)."""
    try:
        mos_product = get_mos_product_instance()
...
@router.get(
    "/configure/{user_id}", summary="Get MemDBProduct configuration", response_model=SimpleResponse
)
def get_config(user_id: str):
    """Get MemDBProduct configuration."""
    global MEMDB_PRODUCT_INSTANCE
    config = MEMDB_PRODUCT_INSTANCE.default_config
...
def update_user_config(user_id: str, config_data: dict):
    ...
    try:
        ...
        # Create MemDBConfig from the provided data
        config = MemDBConfig(**config_data)
