...
from memdb.api.handlers.base_handler import BaseHandler, HandlerDependencies
...
from memdb.mem_os.utils.format_utils import clean_json_response
from memdb.mem_os.utils.reference_utils import (
    prepare_reference_data,
    process_streaming_references_complete,
)
from memdb.mem_reader.read_multi_modal.utils import detect_lang
...
from memdb.templates.cloud_service_prompt import get_cloud_chat_prompt
...
class ChatHandler(BaseHandler):
    ...
    def __init__(
        ...
        if search_handler is None:
            from memdb.api.handlers.search_handler import SearchHandler

            search_handler = SearchHandler(dependencies)

        if add_handler is None:
            from memdb.api.handlers.add_handler import AddHandler
    ...
    def handle_chat_complete(self, chat_req: APIChatCompleteRequest) -> dict[str, Any]:
        """
        Chat with MemDB for chat complete response (non-streaming).
    ...
    def handle_chat_stream(self, chat_req: ChatRequest) -> StreamingResponse:
        """
        Chat with MemDB via Server-Sent Events (SSE) stream for chat stream response.
    ...
    def handle_chat_stream_playground(self, chat_req: ChatPlaygroundRequest) -> StreamingResponse:
        """
        Chat with MemDB via Server-Sent Events (SSE) stream for playground chat stream response.
