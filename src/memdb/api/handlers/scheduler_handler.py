...
def handle_scheduler_allstatus(
    ...
    def _aggregate_counts_from_redis(
        ...
        cursor: int | str = 0
        while True:
            cursor, keys = redis_client.scan(cursor=cursor, match="memdb:task_meta:*", count=200)
            for key in keys:
                h_cursor: int | str = 0
                while True:
...
def handle_task_queue_status(
    ...
    try:
        queue_wrapper = getattr(mem_scheduler, "memdb_message_queue", None)
        ...
        queue = getattr(queue_wrapper, "memdb_message_queue", queue_wrapper)
