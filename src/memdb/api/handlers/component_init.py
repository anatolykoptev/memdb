...
This module handles the initialization of all MemDB server components
...
from memdb.api.handlers.config_builders import (
...
from memdb.mem_os.product_server import MemDBServer
...
from memdb.memories.textual.prefer_text_memory.factory import (
    AdderFactory,
    ExtractorFactory,
    RetrieverFactory,
)
...
from memdb.memories.textual.tree_text_memory.organize.history_manager import MemoryHistoryManager
...
def init_server() -> dict[str, Any]:
    ...
    required by the MemDB server, including:
    ...
    """
    logger.info("Initializing MemDB server components...")

    # Initialize Redis client first as it is a core dependency for features like scheduler status tracking
    if os.getenv("MEMDBSCHEDULER_USE_REDIS_QUEUE", "False").lower() == "true":
        try:
            from memdb.mem_scheduler.orm_modules.api_redis_model import APIRedisDBManager
    ...
    # Initialize MemDB Server
    mos_server = MemDBServer(
        mem_reader=mem_reader,
        llm=llm,
        online_bot=False,
    )

    logger.debug("MemDB server initialized")
    ...
    logger.info("MemDB server components initialized successfully")

    # Initialize online bot if enabled
    online_bot = None
    if dingding_enabled:
        from memdb.memdb_tools.notification_service import get_online_bot_function
