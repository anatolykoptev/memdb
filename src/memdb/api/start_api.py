...
from memdb.api.middleware.request_context import RequestContextMiddleware
...
DEFAULT_CONFIG = {
    ...
    "chat_model": {
        ...
        "config": {
            "model_name_or_path": os.getenv("MEMDB_CHAT_MODEL", "gpt-3.5-turbo"),
...
# Initialize MemDB instance with lazy initialization
MEMDB_INSTANCE = None


def get_mos_instance():
    """Get or create MemDB instance with default user creation."""
    global MEMDB_INSTANCE
    if MEMDB_INSTANCE is None:
        # Create a temporary MemDB instance to access user manager
        temp_config = MemDBConfig(**DEFAULT_CONFIG)
        temp_mos = MemDB.__new__(MemDB)
        ...
        # Now create the actual MemDB instance
        MEMDB_INSTANCE = MemDB(config=temp_config)

    return MEMDB_INSTANCE


app = FastAPI(
    title="MemDB REST APIs",
    description="A REST API for managing and searching memories using MemDB.",
    version="1.0.0",
)
...
@app.post("/configure", summary="Configure MemDB", response_model=ConfigResponse)
async def set_config(config: MemDBConfig):
    ...
    # Now create the MemDB instance
    MEMDB_INSTANCE = MemDB(config=config)
...
@app.post("/chat", summary="Chat with MemDB", response_model=ChatResponse)
async def chat(chat_req: ChatRequest):
    """Chat with the MemDB system."""
    mos_instance = get_mos_instance()
