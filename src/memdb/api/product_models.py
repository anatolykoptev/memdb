...
from memdb.types import MessageList, MessagesType, PermissionDict, SearchMode
...
class ChatRequest(BaseRequest):
    ...
    moscube: bool = Field(
        False,
        description="(Deprecated) Whether to use legacy MemDBCube pipeline.",
    )
    ...
    def _convert_deprecated_fields(self):
        ...
        if self.moscube:
            logger.warning(
                "ChatRequest.moscube is deprecated. Legacy MemDBCube pipeline "
...
class ChatCompleteRequest(BaseRequest):
    ...
    moscube: bool = Field(
        False, description="(Deprecated) Whether to use legacy MemDBCube pipeline"
    )
...
class APISearchRequest(BaseRequest):
    ...
    relativity: float = Field(
        ...
        description=(
            ...
            "Use 0 to disable threshold filtering. Default: 0 (disabled). "
            "PolarDB normalized scores are (cosine+1)/2, typically ~0.5."
        ),
    )
    ...
    moscube: bool = Field(
        False,
        description="(Deprecated / internal) Whether to use legacy MemDBCube path.",
    )
    ...
    def _convert_deprecated_fields(self) -> "APISearchRequest":
        ...
        if self.moscube:
            logger.warning(
                ...
                "Legacy MemDBCube pipeline will be removed soon."
            )
...
# ─── MemDB Response Models (Similar to OpenAI ChatCompletion) ──────────────────


class MemDBGetMessagesResponse(BaseModel):
...
class MemDBSearchResponse(BaseModel):
...
class MemDBDeleteKnowledgebaseResponse(BaseModel):
...
class MemDBDeleteMemoryResponse(BaseModel):
    """Response model for delete knowledgebase operation based on actual API."""
...
class MemDBChatResponse(BaseModel):
...
class MemDBGetTaskStatusResponse(BaseModel):
...
class MemDBCreateKnowledgebaseResponse(BaseModel):
...
class MemDBAddKnowledgebaseFileResponse(BaseModel):
...
class MemDBGetMemoryResponse(BaseModel):
...
class MemDBGetKnowledgebaseFileResponse(BaseModel):
...
class MemDBAddResponse(BaseModel):
...
class MemDBAddFeedBackResponse(BaseModel):
