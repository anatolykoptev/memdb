...
def load_default_config(user_id="default_user"):
    """
    Load MemDB configuration using the same APIConfig-based initialization
...
class MemDBMCPServer:
    """MCP Server that accepts an existing MemDB instance."""

    def __init__(self, mos_instance: MemDB | None = None):
        self.mcp = FastMCP("MemDB Memory System")
        if mos_instance is None:
            # Use APIConfig-based initialization (same as server_router)
            config, cube_config = load_default_config()
            self.mos_core = MemDB(config=config)
    ...
    def _setup_tools(self):
        ...
        async def chat(query: str, user_id: str | None = None) -> str:
            """
            Chat with MemDB system using memory-enhanced responses.
        ...
        async def create_user(
            ...
            """
            Create a new user in the MemDB system.
        ...
        async def register_cube(
            ...
            """
            Register an existing memory cube with the MemDB system.
        ...
        async def unregister_cube(cube_id: str, user_id: str | None = None) -> str:
            """
            Unregister a memory cube from the MemDB system.
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="MemDB MCP Server")
    parser.add_argument(
        "--transport",
