...
from memdb.configs.mem_user import UserManagerConfigFactory


class MemDBConfig(BaseConfig):
    user_id: str = Field(
        default="root",
        description="User ID for MemDB. This is used to distinguish between different users' memories.",
    )
    session_id: str = Field(
        default=str(uuid.uuid4()),
        description="Session ID for MemDB. This is used to distinguish between different dialogue",
    )
    chat_model: LLMConfigFactory = Field(
        ...,
        default_factory=LLMConfigFactory,
        description="LLM configuration for the chat model in MemDB",
    )
    mem_reader: MemReaderConfigFactory = Field(
        ...,
        default_factory=MemReaderConfigFactory,
        description="MemReader configuration for MemDB",
    )
...
class MemDBConfigFactory(BaseConfig):
    """Factory class for creating Memos configurations."""

    config: dict[str, Any] = Field(..., description="Configuration for the MemDB backend")

    @model_validator(mode="after")
    def create_config(self) -> "MemDBConfigFactory":
        self.config = MemDBConfig(**self.config)
        return self
