...
class PostgresGraphDBConfig(BaseConfig):
    """
    PostgreSQL + pgvector configuration for MemDB.

    Uses standard PostgreSQL with pgvector extension for vector search.
    Does NOT require Apache AGE or other graph extensions.

    Schema:
    - memdb_memories: Main table for memory nodes (id, memory, properties JSONB, embedding vector)
    - memdb_edges: Edge table for relationships (source_id, target_id, type)
    ...
    schema_name: str = Field(default="memdb", description="Schema name for MemDB tables")
    user_name: str | None = Field(
        default=None,
