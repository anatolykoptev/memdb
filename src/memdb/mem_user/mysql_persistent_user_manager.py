"""Persistent user management system for MemDB with configuration storage.

This module extends the MySQL UserManager to provide persistent storage
for user configurations and MemDB instances.
"""
...
from memdb.mem_user.mysql_user_manager import Base, MySQLUserManager
...
class UserConfig(Base):
    ...
    config_data = Column(Text, nullable=False)  # JSON string of MemDBConfig
...
class MySQLPersistentUserManager(MySQLUserManager):
    ...
    def __init__(
        ...
        Args:
            ...
            database (str): MySQL database name. Defaults to "memdb_users".
    ...
    def save_user_config(self, user_id: str, config: MemDBConfig) -> bool:
        """Save user configuration to database.

        Args:
            user_id (str): The user ID.
            config (MemDBConfig): The user's MemDB configuration.
    ...
    def list_user_configs(self, limit: int = 1) -> dict[str, MemDBConfig]:
        """List all user configurations.

        Returns:
            Dict[str, MemDBConfig]: Dictionary mapping user_id to MemDBConfig.
        """
        session = self._get_session()
        try:
            ...
            for user_config in user_configs:
                try:
                    ...
                    # Convert datetime strings back to datetime objects
                    config_dict = self._convert_datetime_strings(config_dict)
                    result[user_config.user_id] = MemDBConfig(**config_dict)
                except Exception as e:
    ...
    def create_user_with_config(
        self, user_name: str, config: MemDBConfig, role=None, user_id: str | None = None
    ) -> str:
        """Create a new user with configuration.

        Args:
            user_name (str): Name of the user.
            config (MemDBConfig): The user's configuration.
