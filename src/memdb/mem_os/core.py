...
class MemDBCore:
    """
    The MemDBCore (Memory Operating System Core) class manages multiple MemCube objects and their operations.
    It provides methods for creating, searching, updating, and deleting MemCubes, supporting multi-user scenarios.
    MemDBCore acts as an operating system layer for handling and orchestrating MemCube instances.
    """

    def __init__(self, config: MemDBConfig, user_manager: UserManager | None = None):
        self.config = config
        ...
        logger.info(f"MemDB initialized for user: {self.user_id}")
    ...
    def register_mem_cube(
        ...
        # check the embedder is it consistent with MemDBConfig
        if hasattr(
            ...
            logger.warning(
                f"Cube Embedder is not consistent with MemDBConfig for cube: {mem_cube_id}, will use Cube Embedder: {cube_embedder}"
            )
