...
from memdb.mem_os.core import MemDBCore
from memdb.mem_os.utils.format_utils import (
    clean_json_response,
...
from memdb.mem_os.utils.reference_utils import (
...
from memdb.mem_user.persistent_factory import PersistentUserManagerFactory
from memdb.mem_user.user_manager import UserRole
from memdb.memories.textual.item import (
    TextualMemoryItem,
)
from memdb.templates.mos_prompts import (
...
CUBE_PATH = os.getenv("MEMDB_CUBE_PATH", "/tmp/data/")
...
class MemDBProduct(MemDBCore):
    """
    The MemDBProduct class inherits from MemDBCore and manages multiple users.
    Each user has their own configuration and cube access, but shares the same model instances.
    """

    def __init__(
        self,
        default_config: MemDBConfig | None = None,
        ...
        """
        Initialize MemDBProduct with an optional default configuration.

        Args:
            default_config (MemDBConfig | None): Default configuration for new users
        ...
        # Initialize parent MemDBCore with root config and persistent user manager
        ...
        self.user_configs: dict[str, MemDBConfig] = {}
    ...
    def _restore_user_instances(
        ...
        try:
            ...
            try:
                from memdb.mem_user.persistent_user_manager import UserConfig
    ...
    def _load_user_cubes(
        ...
        for cube in accessible_cubes[:1]:
            if cube.cube_id not in self.mem_cubes:
                try:
                    if cube.cube_path and os.path.exists(cube.cube_path):
                        # Use MemDBCore's register_mem_cube method directly with default config
    ...
    def _get_user_config(self, user_id: str) -> MemDBConfig:
    ...
    def _validate_user_access(self, user_id: str, cube_id: str | None = None) -> None:
        """Validate user access using MemDBCore's built-in validation."""
        # Use MemDBCore's built-in user validation
        if cube_id:
            self._validate_cube_access(user_id, cube_id)
        else:
            self._validate_user_exists(user_id)

    def _create_user_config(self, user_id: str, config: MemDBConfig) -> MemDBConfig:
    ...
    def _get_or_create_user_config(
        self, user_id: str, config: MemDBConfig | None = None
    ) -> MemDBConfig:
    ...
    def _build_base_system_prompt(
        ...
        now = datetime.now()
        formatted_date = now.strftime("%Y-%m-%d (%A)")
        sys_body = get_memdb_prompt(date=formatted_date, tone=tone, verbosity=verbosity, mode=mode)
        prefix = (base_prompt.strip() + "\n\n") if base_prompt else ""
    ...
    async def _post_chat_processing(
        ...
        try:
            ...
            if self.online_bot:
                ...
                try:
                    from memdb.memdb_tools.notification_utils import (
    ...
    def register_mem_cube(
        ...
        """
        Register a MemCube with the MemDB.
        ...
        if isinstance(mem_cube_name_or_path_or_object, GeneralMemCube):
            ...
            if mem_cube_id in self.mem_cubes:
                logger.info(f"MemCube with ID {mem_cube_id} already in MemDB, skip install.")
                return
    ...
    def user_register(
        ...
        Args:
            ...
            config (MemDBConfig | None, optional): User-specific configuration. Defaults to None.
        ...
        try:
            ...
            # Create a default cube for the user using MemDBCore's methods
            ...
            # Register the default cube with MemDB
            self.register_mem_cube(
    ...
    def chat_with_references(
        ...
        if self.config.enable_activation_memory:
            # Handle activation memory (copy MemDBCore logic)
    ...
    def share_cube_with_user(self, cube_id: str, owner_user_id: str, target_user_id: str) -> bool:
        ...
        # Use MemDBCore's built-in cube access validation
    ...
    def update_user_config(self, user_id: str, config: MemDBConfig) -> bool:
        """Update user configuration.

        Args:
            user_id (str): The user ID.
            config (MemDBConfig): The new configuration.
    ...
    def get_user_config(self, user_id: str) -> MemDBConfig | None:
        """Get user configuration.
        ...
        Returns:
            MemDBConfig | None: The user's configuration or None if not found.
        """
