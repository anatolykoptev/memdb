...
class MemDB(MemDBCore):
    """
    The MemDB (Memory Database) class inherits from MemDBCore.
    This class maintains backward compatibility with the original interface.
    """

    def __init__(self, config: MemDBConfig | None = None):
        """
        Initialize MemDB with optional automatic configuration.

        Args:
            config (MemDBConfig, optional): MemDB configuration. If None, will use automatic configuration from environment variables.
        """
        if config is None:
    ...
    def _auto_configure(self, **kwargs) -> tuple[MemDBConfig, Any]:
        """
        Automatically configure MemDB with default settings.

        Returns:
            tuple[MemDBConfig, Any]: MemDB configuration and default MemCube
        """
        # Get configuration from environment variables
        openai_api_key = os.getenv("OPENAI_API_KEY")
        openai_api_base = os.getenv("OPENAI_API_BASE", "https://api.openai.com/v1")
        text_mem_type = os.getenv("MEMDB_TEXT_MEM_TYPE", "general_text")

        if not openai_api_key:
            raise ValueError("OPENAI_API_KEY environment variable is required")

        logger.info(f"Auto-configuring MemDB with text_mem_type: {text_mem_type}")
        return get_default(
    ...
    @classmethod
    def simple(cls) -> "MemDB":
        """
        Create a MemDB instance with automatic configuration from environment variables.

        This is the simplest way to get started with MemDB.

        Environment variables needed:
        - OPENAI_API_KEY: Your OpenAI API key
        - OPENAI_API_BASE: OpenAI API base URL (optional, defaults to "https://api.openai.com/v1")
        - MEMDB_TEXT_MEM_TYPE: Text memory type (optional, defaults to "general_text")

        Returns:
            MemDB: Configured MemDB instance with auto-registered default cube

        Example:
            ```python
            # Set environment variables
            export OPENAI_API_KEY="your-api-key"
            export MEMDB_TEXT_MEM_TYPE="general_text"
    ...
    def chat(self, query: str, user_id: str | None = None, base_prompt: str | None = None) -> str:
        ...
        Returns:
            str: The response from the MemDB.
        """
        ...
        try:
            ...
            if len(accessible_cubes) == 1:
                ...
                if self.enable_mem_scheduler and self.mem_scheduler is not None:
                    ...
                    from memdb.mem_scheduler.schemas import (
                        ANSWER_LABEL,
                        ScheduleMessageItem,
                    )
    ...
    def get_sub_answers(
        ...
        def generate_answer_for_question(question_index: int, sub_question: str) -> tuple[int, str]:
            ...
            # Build system prompt with memories (similar to MemDBCore._build_system_prompt)
            base_prompt = (
