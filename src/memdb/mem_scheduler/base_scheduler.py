...
from memdb.configs.mem_scheduler import AuthConfig, BaseSchedulerConfig
...
from memdb.mem_scheduler.general_modules.init_components_for_scheduler import init_components
from memdb.mem_scheduler.general_modules.misc import AutoDroppingQueue as Queue
from memdb.mem_scheduler.general_modules.scheduler_logger import SchedulerLoggerModule
from memdb.mem_scheduler.memory_manage_modules.retriever import SchedulerRetriever
from memdb.mem_scheduler.monitors.dispatcher_monitor import SchedulerDispatcherMonitor
from memdb.mem_scheduler.monitors.general_monitor import SchedulerGeneralMonitor
from memdb.mem_scheduler.monitors.task_schedule_monitor import TaskScheduleMonitor
from memdb.mem_scheduler.schemas.general_schemas import (
...
from memdb.mem_scheduler.schemas.monitor_schemas import MemoryMonitorItem
...
from memdb.mem_scheduler.task_schedule_modules.orchestrator import SchedulerOrchestrator
from memdb.mem_scheduler.task_schedule_modules.task_queue import ScheduleTaskQueue
from memdb.mem_scheduler.utils import metrics
from memdb.mem_scheduler.utils.db_utils import get_utc_now
...
from memdb.templates.mem_scheduler_prompts import MEMORY_ASSEMBLY_TEMPLATE
from memdb.types.general_types import (
    MemCubeID,
    UserID,
)
...
if TYPE_CHECKING:
    ...
    from memdb.reranker.http_bge import HTTPBGEReranker
...
class BaseScheduler(RabbitMQSchedulerModule, RedisSchedulerModule, SchedulerLoggerModule):
    ...
    def __init__(self, config: BaseSchedulerConfig):
        ...
        self.mem_reader = None  # Will be set by MemDBCore
        self._status_tracker: TaskStatusTracker | None = None
        self.metrics = metrics
        self._monitor_thread = None
        self.memdb_message_queue = ScheduleTaskQueue(
        ...
        self.dispatcher = SchedulerDispatcher(
            config=self.config,
            memdb_message_queue=self.memdb_message_queue,
        ...
        self.task_schedule_monitor = TaskScheduleMonitor(
            memdb_message_queue=self.memdb_message_queue.memdb_message_queue,
    ...
    def initialize_modules(
        ...
        try:
            if redis_client and self.use_redis_queue:
                ...
                if self.memdb_message_queue:
                    # Use the setter to propagate to the inner queue (e.g. SchedulerRedisQueue)
                    self.memdb_message_queue.set_status_tracker(self.status_tracker)
    ...
    def status_tracker(self) -> TaskStatusTracker | None:
        ...
        if self._status_tracker is None and self.use_redis_queue:
            try:
                ...
                if self.memdb_message_queue:
                    self.memdb_message_queue.set_status_tracker(self._status_tracker)
            except Exception as e:
    ...
    def status_tracker(self, value: TaskStatusTracker | None) -> None:
        ...
        try:
            ...
            if self.memdb_message_queue and value is not None:
                self.memdb_message_queue.set_status_tracker(value)
        except Exception as e:
    ...
    def update_activation_memory(
        ...
        try:
            ...
            logger.error(f"MemDB-based activation memory update failed: {e}", exc_info=True)
    ...
    def submit_messages(self, messages: ScheduleMessageItem | list[ScheduleMessageItem]):
        ...
        if queued_msgs:
            self.memdb_message_queue.submit_messages(messages=queued_msgs)
    ...
    def _message_consumer(self) -> None:
        ...
        while self._running:  # Use a running flag for graceful shutdown
            try:
                ...
                messages = self.memdb_message_queue.get_messages(batch_size=self.consume_batch)
    ...
    def _monitor_loop(self):
        while self._running:
            try:
                q_sizes = self.memdb_message_queue.qsize()
    ...
    def _gather_queue_stats(self) -> dict:
        ...
        if not self.use_redis_queue:
            try:
                stats["qsize"] = int(memdb_message_queue.qsize())
            except Exception:
                stats["qsize"] = -1
            # unfinished_tasks if available
            try:
                stats["unfinished_tasks"] = int(
                    getattr(memdb_message_queue, "unfinished_tasks", 0) or 0
                )
            except Exception:
