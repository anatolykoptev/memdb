...
from memdb.configs.mem_scheduler import AuthConfig, RabbitMQConfig
...
from memdb.mem_scheduler.general_modules.misc import AutoDroppingQueue
from memdb.mem_scheduler.schemas.general_schemas import DIRECT_EXCHANGE_TYPE, FANOUT_EXCHANGE_TYPE
...
class RabbitMQSchedulerModule(BaseSchedulerModule):
    ...
    def __init__(self):
        ...
        self.rabbit_queue_name = "memdb-scheduler"
        self.rabbitmq_exchange_name = "memdb-fanout"  # Default, will be overridden by config
    ...
    def initialize_rabbitmq(
        ...
        try:
            ...
            enable_env = os.getenv("MEMDB_ENABLE_RABBITMQ", "").lower() == "true"
            in_ci = os.getenv("CI", "").lower() == "true"
            in_pytest = os.getenv("PYTEST_CURRENT_TEST") is not None
            logger.info(
                f"[DIAGNOSTIC] initialize_rabbitmq called. in_ci={in_ci}, in_pytest={in_pytest}, "
                f"MEMDB_ENABLE_RABBITMQ={enable_env}, config_path={config_path}"
            )
            if (in_ci or in_pytest) and not enable_env:
                logger.info(
                    "Skipping RabbitMQ initialization in CI/test environment. Set MEMDB_ENABLE_RABBITMQ=true to enable."
                )
                return
            ...
            env_exchange_name = os.getenv("MEMDBSCHEDULER_RABBITMQ_EXCHANGE_NAME")
            env_exchange_type = os.getenv("MEMDBSCHEDULER_RABBITMQ_EXCHANGE_TYPE")
    ...
    def rabbitmq_publish_message(self, message: dict):
        ...
        # Env override: apply to all message types when MEMDBSCHEDULER_RABBITMQ_EXCHANGE_NAME is set
        env_exchange_name = os.getenv("MEMDBSCHEDULER_RABBITMQ_EXCHANGE_NAME")
        env_routing_key = os.getenv("MEMDBSCHEDULER_RABBITMQ_ROUTING_KEY")
        if env_exchange_name:
