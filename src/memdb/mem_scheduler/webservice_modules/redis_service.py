...
class RedisSchedulerModule(BaseSchedulerModule):
    ...
    def auto_initialize_redis(self) -> bool:
        ...
        enable_env = os.getenv("MEMDB_ENABLE_REDIS", "").lower() == "true"
        in_ci = os.getenv("CI", "").lower() == "true"
        in_pytest = os.getenv("PYTEST_CURRENT_TEST") is not None
        if (in_ci or in_pytest) and not enable_env:
            logger.info(
                "Skipping Redis auto-initialization in CI/test environment. Set MEMDB_ENABLE_REDIS=true to enable."
            )
            return False
        ...
        try:
            redis_host = os.getenv("MEMDBSCHEDULER_REDIS_HOST", "localhost")
            redis_port = int(os.getenv("MEMDBSCHEDULER_REDIS_PORT", "6379"))
            redis_db = int(os.getenv("MEMDBSCHEDULER_REDIS_DB", "0"))
            redis_password = os.getenv("MEMDBSCHEDULER_REDIS_PASSWORD", None)
            socket_timeout = os.getenv("MEMDBSCHEDULER_REDIS_TIMEOUT", None)
            socket_connect_timeout = os.getenv("MEMDBSCHEDULER_REDIS_CONNECT_TIMEOUT", None)
            ...
            if socket_timeout is not None:
                try:
                    ...
                    logger.warning(
                        f"Invalid MEMDBSCHEDULER_REDIS_TIMEOUT value: {socket_timeout}, ignoring"
                    )
            ...
            if socket_connect_timeout is not None:
                try:
                    ...
                    logger.warning(
                        f"Invalid MEMDBSCHEDULER_REDIS_CONNECT_TIMEOUT value: {socket_connect_timeout}, ignoring"
                    )
