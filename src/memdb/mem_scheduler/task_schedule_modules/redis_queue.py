...
class SchedulerRedisQueue(RedisSchedulerModule):
    ...
    the local memdb_message_queue functionality, offering better scalability
    ...
    def __init__(
        ...
        stream_key_prefix: str = os.getenv(
            "MEMDBSCHEDULER_REDIS_STREAM_KEY_PREFIX",
            DEFAULT_STREAM_KEY_PREFIX,
        ),
        ...
        self._initial_scan_max_keys = int(
            os.getenv("MEMDBSCHEDULER_REDIS_INITIAL_SCAN_MAX_KEYS", "1000") or 1000
        )
        self._initial_scan_time_limit_sec = float(
            os.getenv("MEMDBSCHEDULER_REDIS_INITIAL_SCAN_TIME_LIMIT_SEC", "1.0") or 1.0
        )

        # Pipeline chunk size for XREVRANGE pipelined calls
        self._pipeline_chunk_size = int(
            os.getenv("MEMDBSCHEDULER_REDIS_PIPELINE_CHUNK_SIZE", "200") or 200
        )
