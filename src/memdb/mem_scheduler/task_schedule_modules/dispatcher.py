...
from memdb.mem_scheduler.general_modules.base import BaseSchedulerModule
from memdb.mem_scheduler.general_modules.task_threads import ThreadManager
from memdb.mem_scheduler.schemas.general_schemas import (
    DEFAULT_STOP_WAIT,
)
from memdb.mem_scheduler.schemas.message_schemas import ScheduleLogForWebItem, ScheduleMessageItem
...
from memdb.mem_scheduler.utils.misc_utils import group_messages_by_user_and_mem_cube, is_cloud_env
...
class SchedulerDispatcher(BaseSchedulerModule):
    ...
    def __init__(
        self,
        max_workers: int = 30,
        memdb_message_queue: ScheduleTaskQueue | None = None,
        ...
        self.memdb_message_queue = (
            memdb_message_queue.memdb_message_queue
            if hasattr(memdb_message_queue, "memdb_message_queue")
            else memdb_message_queue
        )
    ...
    def _create_task_wrapper(self, handler: Callable, task_item: RunningTaskItem):
        ...
        def wrapped_handler(messages: list[ScheduleMessageItem]):
            ...
            try:
                ...
                if (
                    isinstance(self.memdb_message_queue, SchedulerRedisQueue)
                    and self.memdb_message_queue is not None
                ):
                    try:
                        for msg in messages:
