...
the local memdb_message_queue functionality in BaseScheduler.
"""

from memdb.context.context import get_current_trace_id
from memdb.log import get_logger
from memdb.mem_scheduler.schemas.message_schemas import ScheduleMessageItem
from memdb.mem_scheduler.task_schedule_modules.local_queue import SchedulerLocalQueue
from memdb.mem_scheduler.task_schedule_modules.orchestrator import SchedulerOrchestrator
from memdb.mem_scheduler.task_schedule_modules.redis_queue import SchedulerRedisQueue
...
class ScheduleTaskQueue:
    def __init__(
        ...
        if self.use_redis_queue:
            if maxsize is None or not isinstance(maxsize, int) or maxsize <= 0:
                maxsize = None
            self.memdb_message_queue = SchedulerRedisQueue(
                max_len=maxsize,
            ...
            self.memdb_message_queue = SchedulerLocalQueue(maxsize=self.maxsize)
    ...
    def set_status_tracker(self, status_tracker: TaskStatusTracker) -> None:
        ...
        """
        self.status_tracker = status_tracker
        if self.memdb_message_queue and hasattr(self.memdb_message_queue, "status_tracker"):
            ...
            self.memdb_message_queue.status_tracker = status_tracker
    ...
    def ack_message(
        ...
        if not isinstance(self.memdb_message_queue, SchedulerRedisQueue):
            logger.warning("ack_message is only supported for Redis queues")
            return

        self.memdb_message_queue.ack_message(
            user_id=user_id,
    ...
    def get_stream_keys(self) -> list[str]:
        if isinstance(self.memdb_message_queue, SchedulerRedisQueue):
            stream_keys = self.memdb_message_queue.get_stream_keys()
        else:
            stream_keys = list(self.memdb_message_queue.queue_streams.keys())
        return stream_keys
    ...
    def submit_messages(self, messages: ScheduleMessageItem | list[ScheduleMessageItem]):
        ...
        for msg in messages:
            ...
            msg.stream_key = self.memdb_message_queue.get_stream_key(
        ...
        if len(messages) < 1:
            ...
            self.memdb_message_queue.put(messages[0])
        else:
            ...
            for _user_id, cube_groups in user_cube_groups.items():
                for _mem_cube_id, user_cube_msgs in cube_groups.items():
                    for message in user_cube_msgs:
                        ...
                        self.memdb_message_queue.put(message)
                        logger.info(
    ...
    def get_messages(self, batch_size: int) -> list[ScheduleMessageItem]:
        return self.memdb_message_queue.get_messages(batch_size=batch_size)

    def clear(self):
        self.memdb_message_queue.clear()

    def qsize(self):
        return self.memdb_message_queue.qsize()
