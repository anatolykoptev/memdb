...
class EvalAnalyzer:
    ...
    def __init__(
        ...
        self.openai_client = OpenAI(
            api_key=openai_api_key or os.getenv("MEMDBSCHEDULER_OPENAI_API_KEY"),
            base_url=openai_base_url or os.getenv("MEMDBSCHEDULER_OPENAI_BASE_URL"),
        )
        self.openai_model = openai_model or os.getenv(
            "MEMDBSCHEDULER_OPENAI_DEFAULT_MODEL", "gpt-4o-mini"
        )
...
def main(version_name="ct-1111"):
    ...
    # Test file paths
    eval_result_dir = f"{BASE_DIR}/evaluation/results/locomo/memdb-api-{version_name}"
    judged_file = os.path.join(eval_result_dir, "memdb-api_locomo_judged.json")
    search_results_file = os.path.join(eval_result_dir, "memdb-api_locomo_search_results.json")
