...
def is_cloud_env() -> bool:
    ...
    - MEMDBSCHEDULER_RABBITMQ_EXCHANGE_NAME must be set to enable cloud behavior.
    - The default memdb-fanout/fanout combination is treated as non-cloud.
    """
    if is_playground_env():
        return False

    exchange_name = _normalize_env_value(os.getenv("MEMDBSCHEDULER_RABBITMQ_EXCHANGE_NAME"))
    exchange_type = _normalize_env_value(os.getenv("MEMDBSCHEDULER_RABBITMQ_EXCHANGE_TYPE"))

    if not exchange_name:
        return False

    return not (
        exchange_name == "memdb-fanout" and (not exchange_type or exchange_type == "fanout")
    )
