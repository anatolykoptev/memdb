# src/memdb/mem_scheduler/utils/status_tracker.py
import json
...
class TaskStatusTracker:
    ...
    def get_all_tasks_global(self) -> dict[str, dict[str, dict]]:
        ...
        cursor: int | str = 0
        while True:
            cursor, keys = self.redis.scan(cursor=cursor, match="memdb:task_meta:*", count=100)
            for key in keys:
                # key format: memdb:task_meta:{user_id}
                parts = key.split(":")
                if len(parts) < 3:
                    continue
                user_id = parts[2]
