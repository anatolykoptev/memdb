...
from memdb.memories.textual.tree_text_memory.retrieve.advanced_searcher import (
...
class TreeTextMemory(BaseTextMemory):
    ...
    def drop(self, keep_last_n: int = 30) -> None:
        ...
        """
        try:
            backup_root = Path(tempfile.gettempdir()) / "memdb_backups"
            backup_root.mkdir(parents=True, exist_ok=True)

            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            backup_dir = backup_root / f"memdb_backup_{timestamp}"
            backup_dir.mkdir()
    ...
    def _cleanup_old_backups(root_dir: Path, keep_last_n: int) -> None:
        ...
        """
        backups = sorted(
            [d for d in root_dir.iterdir() if d.is_dir() and d.name.startswith("memdb_backup_")],
