...
export default function SettingsDialog({ open, onOpenChange, onApiKeySaved }: SettingsDialogProps) {
  ...
  useEffect(() => {
    ...
    const fetchMemoryConfig = async () => {
      try {
        ...
        console.error('Failed to fetch MemDB config:', err);
      }
    };
  ...
  const handleSaveMemoryApiKey = async () => {
    ...
    if (!trimmedKey) {
      setMemoryError('Please enter a MemDB API key.');
      return;
    }
    ...
    try {
      ...
      setMemoryHasApiKey(config.hasApiKey);
      setMemoryApiKeyPrefix(config.apiKeyPrefix || null);
      setMemoryStatus('MemDB API key saved securely.');
    } catch (err) {
      const message = err instanceof Error ? err.message : 'Failed to save MemDB API key.';
  ...
  const handleClearMemoryApiKey = async () => {
    ...
    try {
      ...
      setMemoryStatus('MemDB API key removed.');
    } catch (err) {
      const message = err instanceof Error ? err.message : 'Failed to remove MemDB API key.';
      setMemoryError(message);
    }
  };
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-8 mt-4">
          ...
          {/* Memory (MemDB) Section */}
          <section>
            <h2 className="mb-4 text-base font-medium text-foreground">Memory (MemDB)</h2>
            <div className="rounded-lg border border-border bg-card p-5">
              <p className="mb-5 text-sm text-muted-foreground leading-relaxed">
                Connect MemDB to give the agent long-term memory. When a key is set, relevant
              ...
              <div className="mb-4">
                <div className="mb-2.5 flex flex-wrap items-center justify-between gap-2">
                  ...
                  <a
                    href="https://github.com/MemDBai/MemDB/login/?from=/openwork/"
                    target="_blank"
                    rel="noreferrer"
                ...
                <input
                  data-testid="memdb-api-key-input"
                  type="password"
                  value={memoryApiKey}
                  onChange={(e) => setMemoryApiKey(e.target.value)}
                  placeholder="Your MemDB API key..."
                  className="w-full rounded-md border border-input bg-background px-3 py-2 text-sm"
                />
                {memoryHasApiKey && (
                  <p className="mt-2 text-xs text-muted-foreground">
              ...
              <div className="flex flex-col gap-3">
                <button
                  data-testid="memdb-save-api-key-button"
                ...
                >
                  {savingMemoryKey ? 'Saving...' : 'Save MemDB API Key'}
                </button>
                {memoryHasApiKey && (
                  <button
                    data-testid="memdb-clear-api-key-button"
