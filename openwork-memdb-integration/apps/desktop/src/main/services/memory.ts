...
const DEFAULT_BASE_URL = 'https://api.memdb.io/v1';
...
function resolveMemoryConfig(): MemoryConfig {
  const env = getEnv();
  const envBaseUrl = env.MEMDB_BASE_URL?.trim() || env.MEMDB_API_URL?.trim();
  const envApiKey = env.MEMDB_API_KEY?.trim();
  const storedKey = getApiKey('memdb')?.trim();

  const baseUrl = envBaseUrl || DEFAULT_BASE_URL;
  const apiKey = envApiKey || storedKey || undefined;
  const apiKeyHeader = env.MEMDB_API_KEY_HEADER?.trim()
    || 'Authorization';
  const apiKeyScheme = env.MEMDB_API_KEY_SCHEME?.trim()
    || 'Token';
