...
## Обновление production (deploy-config)
...
## Миграция из текущего deploy-config

Текущий `deploy-config/services/memos-core/` содержит смешанный код.
После перехода на overlay pattern:

1. **deploy-config** будет использовать `Dockerfile.krolik` из форка
2. **Локальные изменения** удаляются из deploy-config
