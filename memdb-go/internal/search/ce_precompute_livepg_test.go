...
func insertRawMemory(t *testing.T, pg *db.Postgres, cubeID, memID, text string) {
	...
	// Insert via a minimal SQL — embedding column is required (halfvec(1024)) but
	// we only test the properties round-trip; use a dummy 1024-dim zero vector.
	emb := "[" + dummyVec(1024) + "]"
...
// dummyVec returns a comma-separated list of n zeros (string form of the
// halfvec literal body).
...
func TestCEPrecompute_PersistAndCascadeClear(t *testing.T) {
	...
	// Seed three raw memories, two of which will list the third in
	// their ce_score_topk so we can assert cascade-clear.
	...
	// Round-trip via raw SQL — confirm the JSON shape parses back into
	// the same neighbor_id/score entries.
	...
	// Cascade-clear: deleting mem-c must wipe ce_score_topk on both
	// mem-a (lists mem-c) and mem-b (lists mem-c).
